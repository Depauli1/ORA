...
contract TroveManagerRates is LiquityBase, Ownable, CheckContract, ITroveManager {
    string constant public NAME = "TroveManagerRates";
    ...
    */
    ...
    // --- ORA rates engine: user-set interest rates ---

    uint constant public MIN_ANNUAL_RATE = 5e15;          // 0.5%
    uint constant public MAX_ANNUAL_RATE = 1e18;          // 100%
    uint constant public RATE_ADJUST_COOLDOWN = 7 days;   // anti redemption-dodging
    uint constant internal SECONDS_IN_ONE_YEAR = 365 days;

    mapping (address => uint) public troveAnnualRate;      // 1e18 = 100% APR
    mapping (address => uint) public troveLastDebtUpdate;  // last interest accrual
    mapping (address => uint) public troveLastRateChange;

    // sum(recordedDebt * annualRate / 1e18): orUSD interest streamed per year (display/APY)
    uint public aggWeightedDebt;

    address public interestRouter;                         // minted interest lands here
    address public redemptionFeeReceiver;                  // redemption ETH fee (treasury)

    event TroveInterestAccrued(address indexed _borrower, uint _interest);
    event TroveRateAdjusted(address indexed _borrower, uint _newRate);
    ...
    **/
    ...
    // --- ORA rates engine functions ---

    // Must be called before setAddresses() (which renounces ownership)
    function setRatesAddresses(address _interestRouter, address _redemptionFeeReceiver) external onlyOwner {
        checkContract(_interestRouter);
        require(_redemptionFeeReceiver != address(0), "TroveManager: fee recv 0");
        interestRouter = _interestRouter;
        redemptionFeeReceiver = _redemptionFeeReceiver;
    }

    function calcPendingTroveInterest(address _borrower) public view returns (uint) {
        if (Troves[_borrower].status != Status.active || troveAnnualRate[_borrower] == 0) { return 0; }
        uint timeDelta = block.timestamp.sub(troveLastDebtUpdate[_borrower]);
        return Troves[_borrower].debt.mul(troveAnnualRate[_borrower]).mul(timeDelta).div(SECONDS_IN_ONE_YEAR).div(DECIMAL_PRECISION);
    }

    /* Lazily accrue a trove's interest: increase its recorded debt and mint the
     * corresponding orUSD to the InterestRouter (backed by the borrower's debt).
     * Permissionless — keepers may poke troves; all state-changing ops call it. */
    function accrueTroveInterest(address _borrower) public {
        if (Troves[_borrower].status != Status.active) { return; }
        uint interest = calcPendingTroveInterest(_borrower);
        troveLastDebtUpdate[_borrower] = block.timestamp;
        if (interest == 0) { return; }
        Troves[_borrower].debt = Troves[_borrower].debt.add(interest);
        aggWeightedDebt = aggWeightedDebt.add(interest.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
        activePool.increaseLUSDDebt(interest);
        lusdToken.mint(interestRouter, interest);
        emit TroveInterestAccrued(_borrower, interest);
    }

    // Called by BorrowerOperations at open, after debt is recorded on the trove
    function initTrove(address _borrower, uint _annualRate) external {
        _requireCallerIsBorrowerOperations();
        _requireValidAnnualRate(_annualRate);
        troveAnnualRate[_borrower] = _annualRate;
        troveLastDebtUpdate[_borrower] = block.timestamp;
        troveLastRateChange[_borrower] = block.timestamp;
        aggWeightedDebt = aggWeightedDebt.add(Troves[_borrower].debt.mul(_annualRate).div(DECIMAL_PRECISION));
        emit TroveRateAdjusted(_borrower, _annualRate);
    }

    function adjustTroveRate(address _borrower, uint _newRate) external {
        _requireCallerIsBorrowerOperations();
        _requireValidAnnualRate(_newRate);
        require(block.timestamp >= troveLastRateChange[_borrower].add(RATE_ADJUST_COOLDOWN),
            "TroveManager: rate cooldown");
        accrueTroveInterest(_borrower); // settle accrued interest at the old rate first
        uint debt = Troves[_borrower].debt;
        _aggSub(debt.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
        aggWeightedDebt = aggWeightedDebt.add(debt.mul(_newRate).div(DECIMAL_PRECISION));
        troveAnnualRate[_borrower] = _newRate;
        troveLastRateChange[_borrower] = block.timestamp;
        emit TroveRateAdjusted(_borrower, _newRate);
    }

    function _requireValidAnnualRate(uint _rate) internal pure {
        require(_rate >= MIN_ANNUAL_RATE && _rate <= MAX_ANNUAL_RATE, "TroveManager: bad rate");
    }

    // aggWeightedDebt is a display aggregate; guard subtractions against rounding dust
    function _aggSub(uint _x) internal {
        aggWeightedDebt = aggWeightedDebt > _x ? aggWeightedDebt - _x : 0;
    }
    ...
    // Single liquidation function. Closes the trove if its ICR is lower than the minimum collateral ratio.
    // Standalone (no batch wrapper): batch sequencing lives in the external
    // BatchLiquidator contract — the per-trove outcome is identical to the
    // in-protocol batch path for a single trove.
    function liquidate(address _borrower) external override {
        ...
        uint LUSDInSP = stabilityPoolCached.getMaxAmountToOffset();
        uint icr = getCurrentICR(_borrower, price);

        LiquidationValues memory single;
        if (_checkRecoveryMode(price)) {
            single = _liquidateRecoveryMode(
                activePoolCached, defaultPoolCached, _borrower, icr, LUSDInSP, _getTCR(price), price);
        } else {
            require(icr < MCR, "TroveManager: nothing to liquidate");
            single = _liquidateNormalMode(activePoolCached, defaultPoolCached, _borrower, LUSDInSP);
        }
        // _liquidateRecoveryMode returns empty values when there is nothing to
        // do (e.g. a healthy trove); the batch path enforced the same via its totals.
        require(single.entireTroveDebt > 0, "TroveManager: nothing to liquidate");

        stabilityPoolCached.offset(single.debtToOffset, single.collToSendToSP);
        _redistributeDebtAndColl(activePoolCached, defaultPoolCached, single.debtToRedistribute, single.collToRedistribute);
        if (single.collSurplus > 0) {
            activePoolCached.sendETH(address(collSurplusPool), single.collSurplus);
        }

        _updateSystemSnapshots_excludeCollRemainder(activePoolCached, single.collGasCompensation);

        uint liquidatedColl = single.entireTroveColl.sub(single.collGasCompensation).sub(single.collSurplus);
        emit Liquidation(single.entireTroveDebt, liquidatedColl, single.collGasCompensation, single.LUSDGasCompensation);

        _sendGasCompensation(activePoolCached, msg.sender, single.LUSDGasCompensation, single.collGasCompensation);
    }
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        accrueTroveInterest(_borrower); // mint pending interest so the burn below matches system debt
    ...
    */
    ...
    // Batch sequencing moved to the external BatchLiquidator contract (this
    // fork implements single-trove liquidation only, to stay under the 24KB
    // contract-size limit). This stub preserves the ITroveManager interface.
    function liquidateTroves(uint _n) external override {
        (_n); // silence unused-parameter warning
        revert("TroveManager: use BatchLiquidator.liquidateTroves");
    }



    // See liquidateTroves stub above.
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        (_troveArray); // silence unused-parameter warning
        revert("TroveManager: use BatchLiquidator.batchLiquidateTroves");
    }
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        if (newDebt == LUSD_GAS_COMPENSATION) {
            ...
            /*
            * Rates engine: the sorted list is keyed by annual interest rate, which a
            * redemption does not change — the trove keeps its position and no
            * re-insertion (and therefore no hint) is needed.
            *
            * If the resultant net debt of the partial is less than the minimum, we bail.
            */
            if (_getNetDebt(newDebt) < MIN_NET_DEBT) {
                singleRedemption.cancelledPartial = true;
                return singleRedemption;
            }

            _aggSub(singleRedemption.LUSDLot.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
    ...
    */
        ...
        override
    {
        ...
        /*
        * ORA rates engine: the sorted list is keyed by annual interest rate, so we walk
        * from the tail — the CHEAPEST borrowers get redeemed first. Paying a higher rate
        * is redemption protection. Troves below MCR are skipped: they are liquidation
        * territory, and redeeming them would realise bad debt.
        * The v1 hint parameters are accepted for ABI compatibility but not needed.
        */
        ...
        while (currentBorrower != address(0) && totals.remainingLUSD > 0 && _maxIterations > 0) {
            ...
            accrueTroveInterest(currentBorrower);
            ...
            if (getCurrentICR(currentBorrower, totals.price) >= MCR) {
                ...
                if (singleRedemption.cancelledPartial) break; // Partial would leave the trove below min net debt
        ...
        // Send the ETH fee to the protocol fee receiver (treasury)
        contractsCache.activePool.sendETH(redemptionFeeReceiver, totals.ETHFee);
    ...
    function _getCurrentTroveAmounts(address _borrower) internal view returns (uint, uint) {
        ...
        uint currentLUSDDebt = Troves[_borrower].debt.add(pendingLUSDDebtReward).add(calcPendingTroveInterest(_borrower));
    ...
    function _applyPendingRewards(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            aggWeightedDebt = aggWeightedDebt.add(pendingLUSDDebtReward.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
        ...
        returns (uint debt, uint coll, uint pendingLUSDDebtReward, uint pendingETHReward)
    {
        ...
        debt = debt.add(pendingLUSDDebtReward).add(calcPendingTroveInterest(_borrower));
    ...
    function _closeTrove(address _borrower, Status closedStatus) internal {
        ...
        _aggSub(Troves[_borrower].debt.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
        troveAnnualRate[_borrower] = 0;
    ...
    */
    ...
    // Rates engine: no one-off borrowing fee — continuous interest replaces it
    ...
    function decayBaseRateFromBorrowing() external override {
        // Rates engine: no borrowing fee, so nothing to decay from borrowing.
        // baseRate is only used (and decayed) by the redemption path.
        _requireCallerIsBorrowerOperations();
    }
    ...
    function increaseTroveDebt(address _borrower, uint _debtIncrease) external override returns (uint) {
        _requireCallerIsBorrowerOperations();
        aggWeightedDebt = aggWeightedDebt.add(_debtIncrease.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
    ...
    function decreaseTroveDebt(address _borrower, uint _debtDecrease) external override returns (uint) {
        _requireCallerIsBorrowerOperations();
        _aggSub(_debtDecrease.mul(troveAnnualRate[_borrower]).div(DECIMAL_PRECISION));
