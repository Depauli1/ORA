...
contract BorrowerOperationsRates is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    string constant public NAME = "BorrowerOperationsRates";
    ...
    // Interface-compat stub: the rates branch requires an annual interest rate at open
    function openTrove(uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        (_maxFeePercentage, _LUSDAmount, _upperHint, _lowerHint); // silence unused warnings
        revert("BorrowerOps: use openTroveWithRate on the rates branch");
    }

    /* Open a trove choosing your own annual interest rate (1e18 = 100%).
    ...
    function openTroveWithRate(uint _LUSDAmount, uint _annualRate, address _upperHint, address _lowerHint) external payable {
        ...
        ITroveManagerRates(address(contractsCache.troveManager)).initTrove(msg.sender, _annualRate);

        // The sorted list is keyed by annual interest rate, not NICR
        sortedTroves.insert(msg.sender, _annualRate, _upperHint, _lowerHint);
    ...
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ...
        // Rates engine: the sorted list is keyed by annual interest rate, which this
        // adjustment does not change — no re-insertion needed.
    ...
    /* Change your trove's annual interest rate. Subject to the TroveManager's
     * 7-day cooldown; accrued interest is settled at the old rate first. */
    function adjustTroveRate(uint _newRate, address _upperHint, address _lowerHint) external {
        _requireTroveisActive(troveManager, msg.sender);
        troveManager.applyPendingRewards(msg.sender); // accrues interest at the old rate
        ITroveManagerRates(address(troveManager)).adjustTroveRate(msg.sender, _newRate);
        sortedTroves.reInsert(msg.sender, _newRate, _upperHint, _lowerHint);
    }
