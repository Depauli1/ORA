 ...
 *
 */
contract StabilityPoolRates is LiquityBase, Ownable, CheckContract, IStabilityPool {
    using SafeMath for uint256;

    string constant public NAME = "StabilityPoolRates";
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        // Rates engine: the sorted list is keyed by interest rate, so the v1
        // lowest-ICR-trove check is not applicable. Withdrawals are always allowed
        // (matching Liquity v2 behaviour).
        uint initialDeposit = deposits[msg.sender].initialValue;
