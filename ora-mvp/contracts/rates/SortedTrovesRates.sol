...
// ORA rates engine: the list key is the trove's annual interest rate, fetched
// live from TroveManagerRates (v1 fetched getNominalICR the same way).
interface ITroveManagerKey {
...
*/
contract SortedTrovesRates is Ownable, CheckContract, ISortedTroves {
    using SafeMath for uint256;

    string constant public NAME = "SortedTrovesRates";
    ...
    ITroveManagerKey public troveManager;
    ...
    function setParams(uint256 _size, address _troveManagerAddress, address _borrowerOperationsAddress) external override onlyOwner {
        ...
        troveManager = ITroveManagerKey(_troveManagerAddress);
    ...
    function insert (address _id, uint256 _NICR, address _prevId, address _nextId) external override {
        ITroveManagerKey troveManagerCached = troveManager;

        _requireCallerIsBOorTroveM(troveManagerCached);
        _insert(troveManagerCached, _id, _NICR, _prevId, _nextId);
    }

    function _insert(ITroveManagerKey _troveManager, address _id, uint256 _NICR, address _prevId, address _nextId) internal {
    ...
    function _validInsertPosition(ITroveManagerKey _troveManager, uint256 _NICR, address _prevId, address _nextId) internal view returns (bool) {
        if (_prevId == address(0) && _nextId == address(0)) {
            // `(null, null)` is a valid insert position if the list is empty
            return isEmpty();
        } else if (_prevId == address(0)) {
            // `(null, _nextId)` is a valid insert position if `_nextId` is the head of the list
            return data.head == _nextId && _NICR >= _troveManager.troveAnnualRate(_nextId);
        } else if (_nextId == address(0)) {
            // `(_prevId, null)` is a valid insert position if `_prevId` is the tail of the list
            return data.tail == _prevId && _NICR <= _troveManager.troveAnnualRate(_prevId);
        } else {
            // `(_prevId, _nextId)` is a valid insert position if they are adjacent nodes and `_NICR` falls between the two nodes' NICRs
            return data.nodes[_prevId].nextId == _nextId &&
                   _troveManager.troveAnnualRate(_prevId) >= _NICR &&
                   _NICR >= _troveManager.troveAnnualRate(_nextId);
        }
    }
     ...
     */
    function _descendList(ITroveManagerKey _troveManager, uint256 _NICR, address _startId) internal view returns (address, address) {
        // If `_startId` is the head, check if the insert position is before the head
        if (data.head == _startId && _NICR >= _troveManager.troveAnnualRate(_startId)) {
     ...
     */
    function _ascendList(ITroveManagerKey _troveManager, uint256 _NICR, address _startId) internal view returns (address, address) {
        // If `_startId` is the tail, check if the insert position is after the tail
        if (data.tail == _startId && _NICR <= _troveManager.troveAnnualRate(_startId)) {
    ...
    function _findInsertPosition(ITroveManagerKey _troveManager, uint256 _NICR, address _prevId, address _nextId) internal view returns (address, address) {
        address prevId = _prevId;
        address nextId = _nextId;

        if (prevId != address(0)) {
            if (!contains(prevId) || _NICR > _troveManager.troveAnnualRate(prevId)) {
        ...
        if (nextId != address(0)) {
            if (!contains(nextId) || _NICR < _troveManager.troveAnnualRate(nextId)) {
    ...
    function _requireCallerIsBOorTroveM(ITroveManagerKey _troveManager) internal view {
