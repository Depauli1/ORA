 ...
 */
contract BranchFeeReceiver is OraOwnable, OraCheckContract {
    string constant public NAME = "BranchFeeReceiver";

    address public troveManagerAddress;
    address public borrowerOperationsAddress;

    uint256 public F_ETH;   // cumulative branch-collateral fees received
    uint256 public F_LUSD;  // cumulative orUSD fees received

    event F_ETHUpdated(uint256 _F_ETH);
    event F_LUSDUpdated(uint256 _F_LUSD);
    event FeesSwept(address _token, address _to, uint256 _amount);
