...
/*
 * ORA Phase 2 — TroveManager with soft liquidations, deployed on new
 * collateral branches (the primary ETH branch keeps the audited v1).
 * Everything upstream is untouched; liquidatePartial() is purely additive.
 */
contract TroveManagerRWA is LiquityBaseRWA, Ownable, CheckContract, ITroveManager {
    string constant public NAME = "TroveManagerRWA";
    ...
    */
    ...
    // --- ORA Phase 2: soft liquidations ---
    //
    // Reduced-penalty partial liquidation in the soft band [105%, 110%):
    // offsets exactly enough debt against the Stability Pool to restore the
    // trove to the MCR, seizing collateral at a 3% premium instead of the
    // ~10% implied by a full liquidation. Borrowers keep their position;
    // Stability Pool losses per unit of restored health shrink. Below the
    // soft floor (or in Recovery Mode) the classic full liquidation applies.

    uint constant public SOFT_LIQ_FLOOR = 103e16;    // 103% — bottom of the soft band (premium-solvent)
    uint constant public SOFT_LIQ_PREMIUM = 103e16;  // collateral seized per unit debt: 1.03 / price

    event TroveSoftLiquidated(address indexed _borrower, uint _debtOffset, uint _collSeized, uint _collToLiquidator);

    struct SoftLiqValues {
        uint price;
        uint icr;
        uint debtToOffset;
        uint remainingDebt;
        uint collSeized;
        uint collToLiquidator;
    }

    function liquidatePartial(address _borrower) external {
        SoftLiqValues memory v;

        v.price = priceFeed.fetchPrice();
        require(!_checkRecoveryMode(v.price)); // no soft liquidations in Recovery Mode

        _applyPendingRewards(activePool, defaultPool, _borrower); // also requires the trove to be active

        v.icr = getCurrentICR(_borrower, v.price);
        require(v.icr < MCR); // trove must be liquidatable
        require(v.icr >= SOFT_LIQ_FLOOR); // below the soft band: use full liquidation

        // x = D * (MCR - ICR) / (MCR - PREMIUM) restores the trove to exactly MCR
        v.debtToOffset = Troves[_borrower].debt.mul(MCR.sub(v.icr)).div(MCR.sub(SOFT_LIQ_PREMIUM));
        v.remainingDebt = Troves[_borrower].debt.sub(v.debtToOffset);
        require(v.remainingDebt >= MIN_NET_DEBT.add(LUSD_GAS_COMPENSATION)); // remainder must stay a valid trove
        require(stabilityPool.getTotalLUSDDeposits() >= v.debtToOffset); // SP must fully absorb the offset

        v.collSeized = v.debtToOffset.mul(SOFT_LIQ_PREMIUM).div(v.price);
        v.collToLiquidator = v.collSeized.div(200); // 0.5% caller incentive

        Troves[_borrower].debt = v.remainingDebt;
        Troves[_borrower].coll = Troves[_borrower].coll.sub(v.collSeized);
        _updateStakeAndTotalStakes(_borrower);

        // Burns debtToOffset of SP deposits and pulls the SP's share of collateral from the ActivePool
        stabilityPool.offset(v.debtToOffset, v.collSeized.sub(v.collToLiquidator));
        // Caller incentive
        activePool.sendETH(msg.sender, v.collToLiquidator);

        // Re-insert at the restored ICR
        sortedTroves.reInsert(
            _borrower,
            LiquityMath._computeNominalCR(Troves[_borrower].coll, v.remainingDebt),
            address(0),
            address(0));

        emit TroveSoftLiquidated(_borrower, v.debtToOffset, v.collSeized, v.collToLiquidator);
        emit TroveUpdated(_borrower, v.remainingDebt, Troves[_borrower].coll, Troves[_borrower].stake, TroveManagerOperation.liquidateInNormalMode);
    }
}
