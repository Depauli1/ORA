...
import "../branches/IBorrowerOperationsERC20.sol";
import "../branches/ICollateralReceiver.sol";
...
contract BorrowerOperationsRWA is LiquityBaseRWA, Ownable, CheckContract, IBorrowerOperationsERC20 {
    ...
    function _pullCollateral(address _from, uint _amount) internal {
        if (_amount == 0) { return; }
        require(collToken.transferFrom(_from, address(this), _amount), "BorrowerOps: collateral transferFrom failed");
    }
    ...
    function moveETHGainToTrove(address _borrower, uint _collAmount, address _upperHint, address _lowerHint) external override {
        ...
        // The Stability Pool transfers _collAmount of collateral to this contract before calling.
    ...
    */
    function _adjustTrove(address _borrower, uint _collDeposit, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ...
        _requireNonZeroAdjustment(_collDeposit, _collWithdrawal, _LUSDChange);
    ...
    function _activePoolAddColl(IActivePool _activePool, uint _amount) internal {
        require(collToken.transfer(address(_activePool), _amount), "BorrowerOps: sending collateral to ActivePool failed");
        ICollateralReceiver(address(_activePool)).receiveCollateral(_amount);
    }
    ...
    function _requireSingularCollChange(uint _collDeposit, uint _collWithdrawal) internal pure {
    ...
    function _requireNonZeroAdjustment(uint _collDeposit, uint _collWithdrawal, uint _LUSDChange) internal pure {
