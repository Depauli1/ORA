...
| New contracts | 🟡 | 189-test suite incl. fuzz; tiered Slither CI gate (Tier 3: 0 high; **Tier 2 forks: high fails unless mechanically PROVEN inherited from an audited base, medium requires written triage** — scripts/slither-gate.js + slither-triage.json); 22KB size gate; gas-snapshot regressions gated; all Tier 3 on Solidity 0.8.24; AUDIT_DIFF.md for auditors | **External audits + bug bounty** — budget item, not a code item (scope: Tier 1–3 incl. guardian + BatchLiquidator) |
| Testing | ✅ | 189-test Hardhat suite (unit + fuzz + fork reads + 8 deploy-pipeline checks + **differential suite**: same op sequences replayed on the v1 engine and the Tier-2 forks, asserting lock-step state, revert parity, conservation invariants and soft-liq exact restoration — test/differential.test.js), 74-test vitest frontend suite (server + UI) + two Playwright e2e tests, ~35-check invariant watcher, Foundry invariant campaigns (256×24 in CI **+ nightly 10k×100 campaign** — .github/workflows/fuzz-nightly.yml), integration scripts, gas snapshots, all in CI | Deeper differential sequences (redemption-path parity); property count in nightly campaigns can always grow |
...
| Frontend | 🟡 | **Pre-flight simulation** (decoded revert before signing, zero gas), **EIP-6963 multi-wallet**, **error tracking** (`/log` ring buffer), **CSP + security headers**, focused Borrow/Earn/Markets sections, oracle/NAV risk banners + stale-data lockout, persistent transaction activity with explorer links, **adaptive polling** (visibility-paused, 8→60s error backoff; one refresh = one ~22-call batch), **centralized locale formatting** (single `NUMBER_LOCALE`), a11y set (focus trap, `role="meter"`, `prefers-contrast`, 44px coarse-pointer targets), stable ethers vendor chunk + documented ≤170KB-gz boot budget, 4 e2e specs (review-dialog open flow, mobile viewport, network-switch degradation, redemption hint pipeline) | WalletConnect relay (needs owner's project ID), IPFS pinning (needs a pinning account), independent usability/accessibility review |
...
| Process | ✅ | 8-job CI on every push (suite + tiered Slither + Foundry + oracle-config + fork reads + keeper load + Tier-3 coverage gate 95% lines / 75% branches + frontend), **nightly fuzz campaign cron**, PR-documented phases, audit-diff doc, runbooks | Second human reviewer :) |
...
- **Fork lock-step**: scripted + 60-op seeded-random sequences replayed on the
  v1 ETH branch and the Tier-2 wstETH branch produce identical protocol state
  to the wei after every op (debts, ICRs, TCR, SP, orUSD supply), with
  identical accept/reject outcomes; conservation invariants (orUSD supply ==
  AP+DP debt; Σ trove debt/coll == AP+DP) hold to sub-dust on both engines
  throughout; `liquidatePartial` restores to exactly MCR.
